#charge_1のふるまい、実行者marker
#damageを与える対象にタグ付け
execute positioned ^ ^ ^0.2 positioned ~-0.25 ~-0.25 ~-0.25 as @e[dx=0] positioned ~-0.5 ~-0.5 ~-0.5 if entity @s[dx=0] run tag @s add yd_cb_target
#自分と例外を除外
...
#damage判定
execute as @s at @e[tag=yd_cb_target] run damage @e[sort=nearest,limit=1] 1 arrow by @p[tag=yd_cb_attacker]
#tag消す
...
execute positioned ^ ^ ^0.2 if score @s yd_cb_while matches 1.. run function yd_cb_main:projectile/tick/1